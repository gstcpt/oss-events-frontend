...
import { getModerators, createModerator, updateModerator, deleteUser, getCompanies } from '@/lib/api/user';
...
export default function Moderators() {
  const t = useTranslations('Dashboard.users.moderators');
  const { user } = useAuth();
  ...
  const fetchAdmins = async () => {
    ...
    try {
      const data = await getModerators(user.id);
      setModerators(data);
    } catch (error) { toast.error(t('errorFetchingModerators')); }
  ...
  const confirmDelete = async () => {
    if (deletingAdmin) {
      try {
        ...
        fetchAdmins();
        toast.success(t('moderatorDeletedSuccessfully'));
      } catch (error) { toast.error(t('errorDeletingModerator')); }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAdmin) {
        ...
        await createModerator(payload, user);
      }
      setIsModalOpen(false);
      fetchAdmins();
      toast.success(editingAdmin ? t('moderatorUpdatedSuccessfully') : t('moderatorCreatedSuccessfully'));
    } catch (error) { toast.error(t('errorSavingModerator')); }
  };
  return (
    <div className="space-y-8">
      <div className="card">
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-lg font-semibold">{t('moderatorsManagement')}</h2>
          <Button onClick={handleAdd} aria-label={t('addNewModerator')} className="addNewBtn"><i className="fa fa-plus mr-2"></i> {t('addNewModerator')}</Button>
        </div>
        <div className="p-6">{loading ? (<div className="text-center py-4">{t('loadingModerators')}</div>) : (<DataTable columns={columns} data={moderators} onEdit={handleEdit} onDelete={handleDelete} defaultSort={{ key: 'id', direction: 'descending' }} />)}</div>
      </div>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingAdmin ? t('editModerator') : t('addNewModerator')}>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title={t('deleteModerator')}>
        <p className="text-gray-700">{t('confirmDeleteModerator')} <strong>{deletingAdmin?.firstname} {deletingAdmin?.lastname}</strong>?</p>
