...
import { X, ChevronLeft, ChevronRight, Image as ImageIcon, Plus } from "lucide-react";
...
export const BlogMediaGallery: React.FC<BlogMediaGalleryProps> = ({
    media,
    initialIndex = 0,
    isOpen,
    onClose
        ...
        <div className="fixed inset-0 z-[999] bg-black/95 flex flex-col items-center justify-center animate-in fade-in duration-500 overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 p-10 flex justify-between items-start z-[1001]">
                <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-bold text-primary uppercase tracking-[0.5em]">{t("gallery")}</span>
                    <h4 className="text-white/40 text-xs font-bold uppercase tracking-widest">
                        {currentIndex + 1} <span className="mx-2">/</span> {media.length}
                    </h4>
                </div>
                <button
                    onClick={onClose}
            ...
            {media.length > 1 && (
                <>
                    <button
                        ...
                        className="absolute left-10 top-1/2 -translate-y-1/2 w-20 h-20 bg-white/5 hover:bg-primary text-white/40 hover:text-white rounded-full flex items-center justify-center z-[1001] transition-all duration-500 backdrop-blur-3xl border border-white/10 hover:border-primary group shadow-2xl"
                    >
                        <ChevronLeft className="w-10 h-10 group-hover:-translate-x-1 transition-transform" />
                    </button>
                    <button
                        onClick={goToNext}
                        className="absolute right-10 top-1/2 -translate-y-1/2 w-20 h-20 bg-white/5 hover:bg-primary text-white/40 hover:text-white rounded-full flex items-center justify-center z-[1001] transition-all duration-500 backdrop-blur-3xl border border-white/10 hover:border-primary group shadow-2xl"
                    >
            ...
            <div className="relative w-full h-full p-20 flex items-center justify-center">
                <div className="relative w-full h-full max-w-7xl">
                    <Image
                        ...
                        alt={`Gallery Full ${currentIndex}`}
                        priority
                    />
                </div>
            </div>

            {/* Footer / Thumbnails rail */}
            <div className="absolute bottom-10 left-0 right-0 flex justify-center gap-4 px-10 overflow-x-auto py-6 z-[1001] no-scrollbar">
                {media.map((item, index) => (
                    <button
                        ...
                        className={`relative flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-4 transition-all duration-500 shadow-2xl ${index === currentIndex
                            ? 'border-primary scale-110 -translate-y-2'
